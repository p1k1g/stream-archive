...
Assert-Match $runtime 'JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE' 'Windows owned-tree cleanup must retain a kill-on-close Job Object.'
Assert-Match $runtime 'AssignProcessToJobObject' 'Windows owned-tree cleanup must assign exact owned processes to the Job Object.'
Assert-Match $runtime 'TerminateJobObject' 'Windows owned-tree cleanup must terminate the retained Job Object.'
Assert-Match $runtime 'CreateToolhelp32Snapshot' 'Windows owned-tree cleanup must discover current descendants independently of root Child liveness.'
Assert-RustTest $runtime 'windows_job_retains_descendant_after_root_exit' 'Windows descendant-survival failure-path test is missing.'
Assert-NotMatch $runtime 'try_wait\(\).*return Ok' 'Windows tree cleanup must not treat root Child exit as proof that descendants are gone.'
...
Assert-Match $runtime 'taskkill\.exe' 'Windows exact-PID taskkill fallback is missing.'
Assert-Match $runtime '\.arg\("/PID"\)' 'Windows termination fallback must target the owned PID.'
Assert-Match $runtime '\.arg\("/T"\)' 'Windows termination fallback must include descendants.'
Assert-Match $runtime '\.arg\("/F"\)' 'Windows termination fallback must force cleanup when required.'
