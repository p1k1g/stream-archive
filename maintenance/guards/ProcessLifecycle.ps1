...
Assert-Match $runtime 'const ATTEMPTS_PER_ROUND:\s*usize\s*=\s*3' 'Windows owned-tree termination must retry transient taskkill failures in bounded rounds.'
Assert-Match $runtime 'loop\s*\{\s*let mut tree_stopped' 'Checked Windows owned-tree termination must keep retrying while the owned child can still be live.'
Assert-Match $runtime 'tokio::time::sleep\(Duration::from_millis\(250\)\)\.await' 'Owned-tree retry rounds must be rate limited rather than spinning.'
Assert-NotMatch $runtime 'failed to terminate owned pid=.*tree after' 'Checked Windows cleanup must not return merely because a taskkill retry budget was exhausted while the child may still be live.'
