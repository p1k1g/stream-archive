...
try {
    ...
    Assert-Match $windowsMetadata 'rev-parse --show-toplevel' 'Windows release metadata must prove the explicit root is the checkout top level.'
    Assert-Match $windowsMetadata 'GITHUB_SHA' 'Windows CI release provenance may use the workflow commit SHA only after checkout validation.'
    Assert-Match $windowsMetadata '\$env:GITHUB_SHA\.ToLowerInvariant\(\) -eq \$headCommit' 'Windows release metadata must match GITHUB_SHA to the checkout HEAD before trusting it.'
    Assert-Match $windowsMetadata 'git -c \$safeDirectoryArgument -C \$resolvedRepositoryRoot status --porcelain --untracked-files=normal' 'Windows release metadata must detect dirty worktrees from the validated repository root.'
