...
try {
    ...
    Assert-Match $windowsMetadata 'GITHUB_SHA' 'Windows CI release provenance must use the workflow commit SHA.'
    Assert-Match $windowsMetadata 'git -C \$resolvedRepositoryRoot status --porcelain --untracked-files=normal' 'Local Windows release metadata must detect dirty worktrees when Git provenance is available.'
