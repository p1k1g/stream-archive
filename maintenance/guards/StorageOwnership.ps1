...
Assert-Match $chzzkVod 'backend\.join\("\.stream-archive"\)\.join\("vod"\)' 'CHZZK VOD must create new temp jobs under the canonical .stream-archive runtime namespace.'
Assert-Match $chzzkVod 'fn legacy_vod_job_root' 'Bounded legacy CHZZK temp cleanup boundary is missing.'
Assert-Match $chzzkVod 'backend\.join\("\.rust-web"\)\.join\("vod"\)' 'Legacy CHZZK temp namespace must remain explicit and cleanup-only.'
Assert-Match $chzzkVod 'cleanup_stale_job_dirs_in_root\(&legacy_vod_job_root\(backend\)\)' 'Legacy CHZZK temp jobs must be scavenged during upgrade without being reused for new jobs.'
...
Assert-RustTest $chzzkVod 'job_root_uses_canonical_runtime_namespace' 'Canonical CHZZK temp runtime namespace regression test is missing.'
Assert-RustTest $chzzkVod 'legacy_stale_chzzk_job_dirs_are_scavenged_without_reuse' 'Legacy CHZZK temp cleanup compatibility regression test is missing.'
