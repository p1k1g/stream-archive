...
Assert-Match $main 'run_headless\(' 'Compatibility server entrypoint must delegate to the shared headless lifecycle.'
Assert-Match $headless 'StreamArchiveCore::open\(&backend_dir\)' 'Headless runtime must bootstrap through StreamArchiveCore.'
Assert-Match $headless 'core\.spawn_vod_history_sync\(\)' 'Headless runtime must keep VOD history synchronization.'
Assert-Match $headless 'core\.spawn_auto_backup\(\)' 'Headless runtime must keep automatic backup scheduling.'
Assert-Match $headless 'core\.spawn_queue_worker\(\)' 'Headless runtime must keep the persistent Queue worker.'
Assert-Match $headless 'core\.shutdown\(\)\.await' 'Headless runtime must shut down through shared owned-runtime cleanup.'
Assert-NotMatch ($main + $headless) '\baxum::|Router::new|TcpListener::bind|include_str!\("\.\./web|STREAM_ARCHIVE_BIND|STREAM_ARCHIVE_TOKEN' 'Headless runtime must stay free of retired Web presentation/listener state.'

# SQLite is the only runtime configuration/history authority.
Assert-Match $headless 'StreamArchiveCore::open\(&backend_dir\)' 'Headless startup must use the canonical shared core.'
...
Assert-NotMatch ($main + $headless) 'bootstrap_primary_once|materialize_primary_files|SOOP_LIVE_SETTING\.ini|SOOP_LIVE_CHANNELS\.txt|SOOP_VOD_SETTING\.ini' 'Runtime must not import or emit legacy INI/TXT mirrors.'
...
Assert-Match $cli 'collect_read_only_preflight' 'CLI doctor must load diagnostics through the shared read-only preflight service.'
Assert-NotMatch $cli 'load_preflight_settings|backup_policy_from_settings|backup_directory_from_settings' 'CLI doctor must not duplicate Store/BackupManager preflight interpretation.'
