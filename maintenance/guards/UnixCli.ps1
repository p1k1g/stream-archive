...
Assert-Match $unixCli 'add_channel' 'Unix channel add must use the shared core atomic mutation.'
Assert-Match $unixCli 'remove_channel' 'Unix channel remove must use the shared core atomic mutation.'
Assert-Match $unixCli 'set_channel_enabled' 'Unix channel enable/disable must use the shared core atomic mutation.'
Assert-Match $store 'pub fn insert_channel' 'Atomic SQLite channel insert boundary is missing.'
Assert-Match $store 'pub fn delete_channel' 'Atomic SQLite channel delete boundary is missing.'
Assert-Match $store 'pub fn set_channel_enabled' 'Atomic SQLite channel enable/disable boundary is missing.'
...
Assert-Match $unixControl '"queue.cancel"' 'Active Queue cancellation must route through the runtime owner.'
Assert-Match $unixControl 'core\.cancel_queue_item' 'Runtime control must execute Queue cancellation on the owner core.'
...
Assert-Match $unixTest 'queue_cancel_reaches_active_runtime_owner' 'Active Queue cancellation owner-routing regression test is missing.'
Assert-Match $store 'atomic_channel_inserts_preserve_changes_from_stale_observers' 'Atomic channel writer regression test is missing.'
