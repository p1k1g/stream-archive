...
Assert-Match $cli 'run_serve' 'CLI serve --watch must delegate to the shared headless lifecycle.'
Assert-Match $headless 'core\.start_watcher\(\)\.await' 'Shared headless lifecycle must start the canonical watcher when requested.'
Assert-Match $server 'STREAM_ARCHIVE_START_WATCHER' 'Compatibility server must retain the canonical watcher startup flag.'
