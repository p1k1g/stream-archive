...
Assert-Match $security 'application.*stream-archive' 'Linux Secret Service entries must remain application-scoped.'
...
Assert-NotMatch $security 'secret-tool[^\r\n]*(?:password|value|secret)\)' 'Linux secret values must never be appended to secret-tool argv.'
