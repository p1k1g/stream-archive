...
$platformRuntime = Read-RepoFile 'rust-web/src/platform_runtime.rs'
...
Assert-Match $chzzkVod 'restrict_private_dir\(&preparing\)' 'CHZZK VOD no longer delegates temporary directory permission hardening to the runtime boundary.'
Assert-Match $platformRuntime 'pub\(crate\) fn restrict_private_dir' 'Runtime private-directory permission hardening helper is missing.'
Assert-Match $platformRuntime 'icacls\.exe' 'Windows current-user-only CHZZK temp ACL setup is missing from the runtime boundary.'
Assert-Match $platformRuntime 'from_mode\(0o700\)' 'Unix current-user-only CHZZK temp directory permissions are missing from the runtime boundary.'
...
Assert-Match $workflow 'rust-web/src/platform_runtime\.rs' 'Phase 18 workflow must trigger when shared runtime ownership primitives change.'
...
Assert-Match $platformRuntime 'pub\(crate\) async fn terminate_owned' 'Shared owned-process termination helper is missing.'
Assert-Match $platformRuntime 'Command::new\("taskkill\.exe"\)' 'Windows owned-process cancellation path is missing from the runtime boundary.'
...
Assert-Match $platformRuntime '\.arg\("/PID"\)' 'CHZZK VOD cancellation is not PID scoped.'
Assert-Match $platformRuntime '\.arg\("/T"\)' 'CHZZK VOD cancellation does not include the owned child tree.'
Assert-NotMatch $platformRuntime 'taskkill[^\r\n]*/IM' 'Shared runtime must never kill processes by image name.'
