...
impl VodQueueManager {
    ...
    /// Start exactly one queue worker for this manager.
    ...
    /// Stop claiming new work, then cancel only the active VOD owned by this queue.
    /// Persisted QUEUED items intentionally remain QUEUED for the next launch.
    ...
    pub async fn enqueue(&self, req: VodDownloadRequest) -> Result<VodQueueItem> {
        if self.stopping.load(Ordering::Acquire) {
            bail!("VOD queue is shutting down");
        }
...
mod tests {
    ...
    fn queue() -> (tempfile::TempDir, VodQueueManager) {
    ...
    async fn queued_item_can_be_cancelled_retried_and_removed() {
        ...
        assert!(queue.retry(&item.id).await.is_err());
        queue.cancel(&item.id).await.unwrap();
    ...
    #[tokio::test]
    async fn restart_marks_active_rows_interrupted() {
        let (_dir, queue) = queue();
        let item = queue.enqueue(request("C:\\SOOP_VOD")).await.unwrap();
        queue
            .conn()
            ...
            .unwrap();
        queue.mark_interrupted().unwrap();
        assert_eq!(queue.item(&item.id).unwrap().unwrap().state, "INTERRUPTED");
        queue.retry(&item.id).await.unwrap();
    }

    #[tokio::test]
    async fn queue_limit_keeps_pending_rows_visible_and_rejects_overflow() {
        let (_dir, queue) = queue();
        ...
        drop(conn);
        assert_eq!(queue.snapshot().await.unwrap().queued_count, QUEUE_LIMIT);
        assert!(queue.enqueue(request("C:\\SOOP_VOD")).await.is_err());
    }

    #[test]
    fn runtime_status_merges_progress_without_losing_identity() {
        let mut item = VodQueueItem {
