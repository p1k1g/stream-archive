use crate::{native_picker, settings_adapter::SettingsDraft, AppState, DiagnosticRow, MainWindow, SettingRow};
...
fn read_snapshot(core: &StreamArchiveCore, include_settings: bool, message: &str) -> Response {
    ...
    Response::Snapshot {
        ...
        channel_count: core.channels().map(|c| c.len().to_string()).unwrap_or_else(|_| "Unavailable".into()),
...
fn worker(requests: mpsc::Receiver<Request>, responses: mpsc::Sender<Response>) {
    let runtime = match tokio::runtime::Builder::new_current_thread().enable_all().build() {
        Ok(runtime) => runtime,
        Err(error) => {
            let _ = responses.send(Response::Error(format!("Worker initialization failed: {error}")));
            return;
        }
    };
    ...
    for request in requests {
        let response = match request {
            Request::Refresh => read_snapshot(&core, false, "Diagnostics refreshed; unsaved edits retained"),
            Request::Reload => read_snapshot(&core, true, "Saved settings reloaded; draft discarded"),
            Request::Save(patch) => match runtime.block_on(core.update_environment_settings(&patch)) {
                Ok(_) => read_snapshot(&core, true, "Saved to canonical SQLite. Active recordings keep their current configuration."),
...
fn render_draft(ui: &MainWindow, draft: &SettingsDraft) {
    let rows: Vec<_> = draft.fields.iter().map(|field| SettingRow {
...
pub fn bind_core_snapshot(ui: &MainWindow, diagnostics: DiagnosticsSnapshot) {
    ...
    let rows: Vec<_> = diagnostics.items.into_iter().map(|item| DiagnosticRow {
        name: item.name.into(),
...
fn send(ui: &MainWindow, sender: &mpsc::Sender<Request>, request: Request) {
    ...
    match sender.send(request) {
        ...
        Err(_) => state.set_settings_message("Runtime worker unavailable; fix the startup error and restart".into()),
    }
}
...
pub fn bind(ui: &MainWindow) -> Controller {
    ...
    let state = ui.global::<AppState>();
    state.set_settings_busy(true);
    if let Err(error) = std::thread::Builder::new().name("native-settings".into()).spawn(move || worker(requests, responses)) {
    ...
    state.on_setting_edited(move |index, value| {
        if let Some(ui) = weak.upgrade() {
            if index >= 0 && !ui.global::<AppState>().get_settings_busy() {
                ...
                ui.global::<AppState>().set_settings_dirty(!draft.patch().is_empty());
            }
        }
    });
    let weak = ui.as_weak();
    ...
    state.on_save_settings(move || {
        if let Some(ui) = weak.upgrade() {
            send(&ui, &save_sender, Request::Save(save_draft.borrow().patch()));
        }
    });
    let weak = ui.as_weak();
    ...
    state.on_pick_setting(move |index| {
        if let Some(ui) = weak.upgrade() {
            if let Some(field) = usize::try_from(index).ok().and_then(|index| pick_draft.borrow().fields.get(index).cloned()) {
                send(&ui, &sender, Request::Pick(index as usize, field.kind, field.value));
            }
        }
    });
    let weak = ui.as_weak();
    ...
    timer.start(TimerMode::Repeated, Duration::from_millis(50), move || {
        ...
        while let Ok(response) = receiver.try_recv() {
            ...
            state.set_settings_busy(false);
            match response {
                Response::Snapshot { fields, diagnostics, backend, database, channel_count, message } => {
                ...
                Response::Picked(index, path) => {
                    if let Some(path) = path {
                        ...
                        render_draft(&ui, &draft.borrow());
                        state.set_settings_message("Selection added to draft; Save validates and persists it".into());
                    } else {
