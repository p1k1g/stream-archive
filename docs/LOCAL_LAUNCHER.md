# SOOP Recorder local launcher

Phase 9 adds a Windows launcher for the default local-use workflow.

## Normal use

1. Double-click `RUN.bat` or `soop-launcher.exe`.
2. If SOOP Recorder is not running, the launcher starts `soop-server.exe` in its own console window.
...
The server console remains visible on purpose. Press `Ctrl+C` in that console to stop SOOP Recorder cleanly. The Rust server then stops only its owned LIVE/VOD child processes through the existing Recorder/VOD lifecycle.

Do not use broad process-name termination such as `taskkill /IM ffmpeg.exe` or `taskkill /IM streamlink.exe`.

## Port conflict

The launcher verifies that the local listener on the configured port is actually SOOP Recorder. If another program is using the port, it shows an error instead of opening the wrong application.
