...
function showSetup(){
  const root=makeOverlay();root.hidden=false;root.innerHTML=`<div class="p10-card"><h2>관리자 계정 만들기</h2><p>처음 한 번만 사용할 SOOP Recorder 관리 계정을 만듭니다.</p><form><label>아이디<input name="username" autocomplete="username" minlength="3" maxlength="32" required placeholder="admin"></label><label>비밀번호<input name="password" type="password" autocomplete="new-password" minlength="12" maxlength="128" required></label><label>비밀번호 확인<input name="confirm" type="password" autocomplete="new-password" minlength="12" maxlength="128" required></label><p class="hint">비밀번호는 12자 이상이어야 합니다. 저장 시 단방향 PBKDF2-SHA256 해시로 보관됩니다.</p><div class="p10-error"></div><div class="p10-actions"><button type="submit">관리자 생성</button></div></form></div>`;
...
function showPasswordChange(){
  const root=makeOverlay();root.hidden=false;root.innerHTML=`<div class="p10-card"><h2>비밀번호 변경</h2><form><label>현재 비밀번호<input name="current" type="password" autocomplete="current-password" required></label><label>새 비밀번호<input name="next" type="password" autocomplete="new-password" minlength="12" maxlength="128" required></label><label>새 비밀번호 확인<input name="confirm" type="password" autocomplete="new-password" minlength="12" maxlength="128" required></label><div class="p10-error"></div><div class="p10-actions"><button type="button" class="cancel">취소</button><button type="submit">변경</button></div></form></div>`;
