...
use crate::{
    primary_config::{validate_setting_updates, validate_vod_tool_updates, VOD_TOOL_KEYS},
...
const FIELDS: &[(&str, SettingKind, &str)] = &[
    ("STREAMLINK_PATH", SettingKind::Executable, "Streamlink executable; blank or AUTO uses discovery"),
    ("STREAMLINK_FALLBACK", SettingKind::Executable, "Fallback Streamlink executable; blank or AUTO uses discovery"),
    ("YT_DLP_PATH", SettingKind::Executable, "yt-dlp executable; blank uses runtime discovery"),
    ("FFMPEG_PATH", SettingKind::Executable, "FFmpeg executable; blank uses runtime discovery"),
    ("OUTPUT_DIR", SettingKind::Directory, "Default LIVE output folder; blank keeps the runtime default"),
    ("CHECK_INTERVAL", SettingKind::Text, "LIVE polling interval in seconds (1–86400)"),
    ("MIN_FREE_SPACE_GB", SettingKind::Text, "Minimum free disk space in GB (0–1000000)"),
    ("QUALITY", SettingKind::Text, "LIVE quality, for example best"),
];

pub fn snapshot(values: &BTreeMap<String, String>) -> Vec<EnvironmentSetting> {
    FIELDS.iter().map(|(key, kind, description)| EnvironmentSetting {
        key: (*key).into(),
...
mod tests {
    ...
    fn accepts_runtime_defaults_and_valid_numeric_settings() {
        for (key, value) in [("STREAMLINK_PATH", "AUTO"), ("YT_DLP_PATH", ""), ("OUTPUT_DIR", ""), ("CHECK_INTERVAL", "30"), ("MIN_FREE_SPACE_GB", "1.5")] {
    ...
    fn rejects_invalid_missing_and_wrong_kind_paths() {
        ...
        for (key, path) in [("OUTPUT_DIR", file), ("FFMPEG_PATH", dir.path().to_path_buf()), ("OUTPUT_DIR", dir.path().join("missing")), ("FFMPEG_PATH", dir.path().join("missing.exe"))] {
